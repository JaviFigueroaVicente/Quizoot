        ...
        <div class="col-12">
            <div class="card">
                <DataTable v-model:filters="filters" :value="formulariosRespondidos" paginator :rows="5"
                            :globalFilterFields="['user_id', 'formulario_id', 'score', 'created_at', 'updated_at']" stripedRows size="small">
                    <template #header>
                        <Toolbar pt:root:class="toolbar-table">
                            <template #start>
                                ...
                                <Button type="button" icon="pi pi-refresh" class="h-100 ml-1" outlined @click="getFormulariosRespondidos()" />
                            </template>
                        </Toolbar>
                    </template>
                    ...
                    <Column class="pe-0 me-0 icon-column-2">
                        <template #body="slotProps">
                            <router-link :to="{ name: 'formulariosRespondidos.edit', params: { id: slotProps.data.formulario_id, id: slotProps.data.user_id } } ">
...
const {getFormulariosRespondidos, deleteFormularioRespondido, formulariosRespondidos } = useFormulariosRespondidos();
const {can} = useAbility()
const filters = ref();

onMounted(() => {
    getFormulariosRespondidos().then(() => {
